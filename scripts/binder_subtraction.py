...
def closeness(omega, b_range, quant_idx=QUANT_IDX):
    ...
    for b_idx, B in enumerate(b_range):
        ...
        for i, DT in enumerate(DT_LIST):
            ...
            if PLOT_DURING:
                plt.errorbar(DT[:, 1], Y, yerr=DT[:, quant_idx + 30],label=str(DT[0, 0]))
            lines[i, 0, :] = DT[:, 1]
            lines[i, 1, :] = Y[:]
        ints = []
        ints[:] = []
...
print(one_min_b)
PLOT_DURING = True
closeness(0.78,np.linspace(best_min_b*0.9, best_min_b*1.1, 25))
closeness(1, np.linspace(one_min_b*0.9, one_min_b*1.1, 25))
