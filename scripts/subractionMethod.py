...
for factor in np.linspace(0.9, 1.1, 50):
    ...
    for dt in datalist:
        sub_dt = dt.copy()
        sub_dt[:, 22] = sub_dt[:, 22] - A_b
        # B == 22
        sub_dt[:, 26] = sub_dt[:, 26] - A_r
        # RS == 26
    ...
    a, res, b, c, d = np.polyfit(
        np.log(combined[:, 0]), np.log(-combined[:, 22]), 1, full=True
    )
    a, resr, b, c, d = np.polyfit(
        np.log(combined[:, 0]), np.log(-combined[:, 26]), 1, full=True
    )
