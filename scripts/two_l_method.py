...
DATAFILES = [
    settings.datatables_path + DTSET + "/datatable_4.0jun113DXY.npy",
    settings.datatables_path + DTSET + "/datatable_8.0jun113DXY.npy",
    settings.datatables_path + DTSET + "/datatable_16.0jun113DXY.npy",
    settings.datatables_path + DTSET + "/datatable_32.0jun113DXY.npy",
    settings.datatables_path + DTSET + "/datatable_64.0jun113DXY.npy",
    settings.datatables_path + DTSET + "/datatable_128.0jun113DXY.npy",
]
JACKFILES = [
...
for index, data1, data2 in zip(
    ...
    TWO_L_QUANT[index, :, :5] = twoLana.twoLomega(
        np.load(data1), np.load(data2)
        )
    # jackknifing
    ...
    for i in range(JACK_NUM):
        j_result.append(
            twoLana.twoLomega(
                j_data[i::JACK_NUM, :], j_data_2[i::JACK_NUM, :]
            )
        )

    j_result = np.array(j_result)
    np.save(
        settings.pickles_path + "/2Lquant/jackknife/jack_" +
        TAG + str(int(j_result[0, 0, 0])) + "_" + str(int(j_result[0, 0, 1])),
        j_result
        )
...
for i in range(len(DATAFILES) - 1):
    # save to npy
    np.save(
        settings.pickles_path+"/2Lquant/" +
        TAG + str(int(TWO_L_QUANT[i, 0, 0])) + "_" + str(int(TWO_L_QUANT[i, 0, 1])),
        TWO_L_QUANT[i, :, :]
        )
