...
def get_M0MT(subser, d, am, am2):
    xmag1 = np.multiply(subser[:, d:, 2], subser[:, d:, 2])
    ymag1 = np.multiply(subser[:, d:, 3], subser[:, d:, 3])
    mag21 = np.add(xmag1, ymag1)
    mag1 = np.sqrt(mag21)

    xmag2 = np.multiply(subser[:, :-d, 2], subser[:, :-d, 2])
    ymag2 = np.multiply(subser[:, :-d, 3], subser[:, :-d, 3])
    mag22 = np.add(xmag2, ymag2)
    mag2 = np.sqrt(mag22)
