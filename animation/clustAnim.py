    ...
    for x in np.unique(X):
        for y in np.unique(Y):
            for z in np.unique(Z):
                ...
                plt.gcf().gca().plot_surface(
                    x + rel_x, y + rel_y, z, color="b", alpha=0.7
                )
...
def dashedLines(X, Y, Z):
    for x in np.unique(X):
        for y in np.unique(Y):
            for z in np.unique(Z):
                plt.gcf().gca().plot(
                    [x - 1, x + 1], [y, y], [z, z], color="tab:grey", alpha=0.2
                )
                plt.gcf().gca().plot(
                    [x, x], [y - 1, y + 1], [z, z], color="tab:grey", alpha=0.2
                )
                plt.gcf().gca().plot(
                    [x, x], [y, y], [z - 1, z + 1], color="tab:grey", alpha=0.2
                )
...
def saveFigure(pngindex):
    ...
    plt.savefig(
        "./png/" + sys.argv[1] + "Frame%03i" % pngindex + ".png", bbox_inches="tight"
    )
