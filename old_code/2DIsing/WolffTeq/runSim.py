...
for L in [4,8,16,32]:
    ...
    for N_sweeps in [2,4,8,16,32,64,128]:
        t1 = []
        t0 = []
        m1 = []
        m0 = []
        for n in range(N):
            runM0 = subprocess.run(["./a.out",str(L),str(T),str(N_sweeps),str(0)],stdout=subprocess.PIPE)
            outM0 = runM0.stdout.decode('utf-8')
            runM1 = subprocess.run(["./a.out",str(L),str(T),str(N_sweeps),str(1)],stdout=subprocess.PIPE)
...
done = subprocess.run(["notify-send","runSim.py finished"])
