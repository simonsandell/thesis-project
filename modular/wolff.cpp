...
int growCluster(double ***lattice,bool ***cluster, double &L,double &beta, double& TotXMag,double& TotYMag,double& TotEn,double &TotSinX,std::uniform_real_distribution<double> &dist,std::mt19937_64 &eng){
	...
	while (n > 0){
		...
		if (cluster[std::get<0>(current)][std::get<1>(current)][std::get<2>(current)] == 0){
			...
			if (dist(eng) < prob){
				...
				if (cluster[std::get<0>(neig1)][std::get<1>(neig1)][std::get<2>(neig1)] ==0){
					perimeter[n] = neig1;
					n = n + 1;
				}
				if (cluster[std::get<0>(neig2)][std::get<1>(neig2)][std::get<2>(neig2)] ==0){
					perimeter[n] = neig2;
					n = n + 1;
				}
				if (cluster[std::get<0>(neig3)][std::get<1>(neig3)][std::get<2>(neig3)] ==0){
					perimeter[n] = neig3;
					n = n + 1;
				}
				if (cluster[std::get<0>(neig4)][std::get<1>(neig4)][std::get<2>(neig4)] ==0){
					perimeter[n] = neig4;
					n = n + 1;
				}
				if (cluster[std::get<0>(neig5)][std::get<1>(neig5)][std::get<2>(neig5)] ==0){
					perimeter[n] = neig5;
					n = n + 1;
				}
				if (cluster[std::get<0>(neig6)][std::get<1>(neig6)][std::get<2>(neig6)] ==0){
					perimeter[n] = neig6;
					n = n + 1;
				}
			}
