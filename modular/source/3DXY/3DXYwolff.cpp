...
void print_spin_id(unsigned long int k, Lattice3DXY &lat){
    std::tuple<int,int,int> xyz = lat.kToXYZ(k);
    //std::cout << std::get<0>(xyz) << std::get<1>(xyz) <<std::get<2>(xyz) << std::endl;
}
...
long double cluster3DXY(Lattice3DXY& lat){
    ...
    while (n > 0){
        ...
        print_spin_id(std::get<0>(current), lat);
        perimeter.pop_back();
        n -= 1;
