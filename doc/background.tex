...
Throughout this chapter we set $k_B=1$.
\section{Models} % define equations, define quantities.
The principle of universality allows one to freely choose a model from the pool of models all coressponding to the same universality class, since all those models will have the same universal critical properties.
...
The class of the lamba transition in $^4$He is represented by the symmetry group $O(2)$, short range interactions and $d=3$. With $d=2$ instead the class changes to that of the famous Kosterlitz-Thouless-transition.
...
\begin{align}
  ...
  \label{}
\end{align}
In the regular liquid phase, the energy of the system is unaffected by uniform boundary motion. The superfluid stiffness is a quantity which measures the shift in energy due to a shift of the angles along a boundary, so this quantity is expected to 
\section{Scaling}
