...
The XY-model and the Ising-model are both parth of the orthogonal symmetry group, written $O(n)$, with $n=2,1$ respectively. The two other properties which determine the universality class of a model is the physical dimensions and the range of interactions. For both models we employ physical dimension $d =3 $ and include only nearest neighbour interactions.

For both models we use a cubic lattice. The hamiltonian of the 3DXY-model is
\begin{equation}
